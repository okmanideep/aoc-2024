...
const ClawMachine = struct {
    ...
    fn print(self: ClawMachine) void {
...
fn parseClawMachine(lines: [3][]const u8) !ClawMachine {
    ...
    const px = try std.fmt.parseInt(usize, px_as_string, 10) + 10000000000000;
    const py_as_string = prizeTokenizer.next() orelse unreachable;
    const py = try std.fmt.parseInt(usize, py_as_string, 10) + 10000000000000;
...
test "parseClawMachine" {
    ...
    try std.testing.expectEqualDeep(ClawMachine{ .ax = 94, .ay = 34, .bx = 22, .by = 67, .px = 10000000008400, .py = 10000000005400 }, claw_machine_1);
}
...
test "aoc example" {
    ...
    try std.testing.expectEqualDeep(ClawMachine{ .ax = 94, .ay = 34, .bx = 22, .by = 67, .px = 10000000008400, .py = 10000000005400 }, list.items[0]);

    try std.io.getStdOut().writer().print("2st machine result: {d}\n", .{list.items[1].leastTokensForPrize()});
}
